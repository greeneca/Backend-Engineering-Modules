...
func Test_processLine(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			line: `data: {"meta":{"uri":"https://en.wikipedia.org/wiki/Special:Diff/1234567890"},"bot":true,"user":"BotUser"}`,
			stats: statstics.Statstics{Messages: 0,Urls: make(map[string]bool), Bots: make(map[string]bool), NonBots: make(map[string]bool),},
			messages: 0,
			Urls: 1,
			Bots: 1,
			...
			line: `data: {"meta":{"uri":"https://en.wikipedia.org/wiki/Special:Diff/0987654321"},"bot":false,"user":"NonBotUser"}`,
			stats: statstics.Statstics{Messages: 0,Urls: make(map[string]bool), Bots: make(map[string]bool), NonBots: make(map[string]bool),},
			messages: 0,
			Urls: 1,
			Bots: 0,
			...
			line: `data: {"meta":{"uri":"https://en.wikipedia.org/wiki/Special:Diff/1122334455"}}`,
			stats: statstics.Statstics{Messages: 0,Urls: make(map[string]bool), Bots: make(map[string]bool), NonBots: make(map[string]bool),},
			messages: 0,
			Urls: 1,
			Bots: 0,
